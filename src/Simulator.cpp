...
// Deletes all created objects (m_obstacles[i])
Simulator::~Simulator(void) {
...
double PointSegmentDistanceSquare(const double p[], const double s0[], const double s1[], double pmin[]);
double PointPolygonDistanceSquare(const double p[], const int n, const double poly[], double pmin[]);
...
// Reads obstcales from a given Obstacles*.txt
...
double PointSegmentDistanceSquare(const double p[], const double s0[], const double s1[], double pmin[]) {
    ...
    if((b = (v[0] * v[0] + v[1] * v[1])) <= a) {
		...
		return (p[0] - s1[0]) * (p[0] - s1[0]) + (p[1] - s1[1]) * (p[1] - s1[1]);
    }
