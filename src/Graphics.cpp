...
void Graphics::HandleEventOnTimer(void) {	
    if(m_setRobotAndGoalCenters == 2) {
		...
		if(!m_simulator.HasRobotReachedGoal()) {
			...
			else if(m_bugSelection == 1)    move = m_bugAlgorithms->Bug1(sensor);
...
// Helper message to give a user what to do to initialize.
...
// main method
// argc = obstacle coordinate text
// argv = bug_argv algorithm that's ran
